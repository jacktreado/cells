...
# name strings
basestr=gelVarPerim_N"$NCELLS"_NV"$NV"_gr"$gelRate"_vp"$varPerimRate"_pt"$phiTarget"_a"$aGelation"
...
binf=bin/"$runstr".o
mainf=$maindir/gelVarPerimRate.cpp
echo Running $numSeeds sims of $NCELLS cells from initially dilute, gel decompression rate = $gelRate, vary perim rate = $varPerimRate, pt = $phiTarget, attraction $aGelation, will compress then decompress to target phi
...
for seed in `seq $startSeed $numSeedsPerRun $endSeed`; do
    ...
    for ss in `seq 0 $ssMax`; do
        ...
        # append to runString
        runString="$runString ; ./$binf $NCELLS $NV $sizeDisp $phiTarget $gelRate $varPerimRate $kl $ka $kb $del $aGelation $runseed $posf $enf $contactf"
    done
