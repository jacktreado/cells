...
simtypedir=$outputdir/tumorInvasion
...
# name strings
basestr=tumor_N"$NCELLS"_NV"$NV"_tN"$tN"_tca"$tca"_gam"$gam"_Dr"$Dr"_NT"$NT"
runstr="$basestr"_i"$startSeed"_f"$endSeed"
...
binf=bin/"$runstr".o
mainf=$maindir/active/adipose2D.cpp
echo Running $numSeeds tumor invasion sims of $NCELLS cells, $tN tumor cells, with $NV verts, tumor calA0 = $calA0, and WAT gam = $gam
