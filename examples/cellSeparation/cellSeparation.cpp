/*
	...
	on two cells sticking initially, then separating
...
int main(){
	...
	string vertexPositionsStr = "/Users/JackTreado/Jamming/Flowers/sim/cells/examples/cellSeparation/cellSeparationPositions.dat";
	string energyStr = "/Users/JackTreado/Jamming/Flowers/sim/cells/examples/cellSeparation/cellSeparationEnergy.dat";
	...
	double dt = 0.075*timeScale;

	// use damping 
	double dampingParam = 1.95*sqrt(segmentMass*kint);
	...
	for (t=0; t<NT; t++){
		...
		// cell shrinking
		if (cell1.totalKineticEnergy() + cell2.totalKineticEnergy() < 1e-4){
			cell1.scale(scaleParam);
			cell2.scale(scaleParam);
		}
	...
	cout << "test code cellSeparation.cpp is finished, returning 0" << endl;
