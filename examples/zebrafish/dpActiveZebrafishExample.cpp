...
// disk constants
const double phiInit 			= 0.7;			// initial packing fraction of disks (sets boundary)
const double phiTarget	 		= 0.98;			// target packing fraction of dp particles
...
int main()
{
	...
	cout << "	** Initializing DP particles in zebrafish boundary with R0 = " << R0 << endl;
	packingObject.dpInitializeActiveZebrafish(NV,phiInit,sizeDispersion,R0);
	...
	packingObject.dpActiveZebrafishNVE(v0);

	// set new force scales
	packingObject.gelForceVals(calA0,kl,ka,gam,1e-4*kb,kint,del,a);

	// dp zebrafish compression
	packingObject.dpActiveZebrafishIsoCompression(Ptol, phiTarget, dphi);
