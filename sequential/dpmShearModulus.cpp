/* 
	...
	** PRINTS VDOS INFORMATION TO FILE
		AT JAMMING ONSET

	** COMPUTES SHEAR MODULUS USING NON-AFFINE VELOCITIES,
		COMPARES TO TRADITIONAL FINITE DIFFERENCE CHECK 

	Jack Treado
	03/13/2021, in the time of covid
...
int main(int argc, char const *argv[]){
	...
	// open shear info file
	ofstream shearout;
	shearout.open(shearFile.c_str());
	if (!shearout.is_open()){
		cout << "	** ERROR: shear file " << shearFile << " could not be opened, ending." << endl;
		return 1;
	}
	...
	// get eigenvalues
	Eigen::VectorXd evals = allModes.eigenvalues();
	...
	// compute affine forces from Hessian
	for (gi=0; gi<NVTOT; gi++){
		kx = NDIM*gi;
		ky = kx + 1;
	...
	// compute non-affine contribution to shear modulus
 
	// get first non-zero mode by eigenvalue magnitude
	int firstNonZeroMode = 0;
	for (i=0; i<vertDOF; i++){
		if (abs(evals(i)) < 1e-10)
			firstNonZeroMode++;
	}

	// project eigenvectors onto Fa direction, add to non-affine contribution
	Gna = 0.0;
	double evproj = 0.0;
	for (i=firstNonZeroMode; i<vertDOF; i++){
		// eigenvector projection
		evproj = 0.0;
		for (j=0; j<vertDOF; j++)
			evproj += Fa[j]*evecs(j,i);

		// add to Gna
		Gna -= pow(evproj,2.0)/evals(i);
	}

	// compute prediction for shear modulus
	// from affine and non-affine contributions
	G = Ga + Gna;
